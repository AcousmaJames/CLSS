...
from trainer.local_D_assigner import DynamicSoftLabelAssigner
...
class SSODTrainer(Trainer):
    def __init__(self, cfg, device, callbacks, LOCAL_RANK, RANK, WORLD_SIZE):
        ...
        if cfg.SSOD.pseudo_label_type == 'FairPseudoLabel':
            ...
            self.pseudo_label_creator = LabelMatch(cfg, int(self.unlabeled_dataset.__len__() / self.WORLD_SIZE),
                                                   self.label_num_per_image, cls_ratio_gt=self.cls_ratio_gt)
    ...
    def splitPreModelCkpt(self,ckpt):
        newckpt = {}
        invalid = [ 'g.0.weight', 'g.1.weight', 'g.1.bias', 'g.1.running_mean', 'g.1.running_var', 'g.1.num_batches_tracked', 'g.3.weight', 'g.3.bias']
        for k,v in ckpt.items():
            if k not in invalid:
                if k.split(".")[0] == 'f':
                    newK = k.replace('f', 'backbone', 1)
                    newckpt[newK] = v.float()
                else:
                    newckpt[k] = v.float()
        return newckpt

    def print_different_keys(self,dict1, dict2):
        """
        打印两个字典中不同的键。
        假设两个字典具有相同的数据类型。
        """
        ...
        set2 = set(dict2.keys())
        # 打印出set1中的独有元素
        print("In 本地 but not in 预训练:")
        # for key in set1 - set2:
        #     print(key)
        print(sorted(set1 - set2))
        # 打印出set2中的独有元素
        print("In 预训练 but not in 本地:")
    ...
    def build_model(self, cfg, device):
        ...
        if pretrained:
            ...
            # csd = ckpt['state_dict'].float().state_dict()
            csd = self.splitPreModelCkpt(ckpt)
            if cfg.prune_finetune:
                dynamic_load(self.model, csd)
                self.model.info()
            aaa = self.model.state_dict()
            newaaa = {}
            for k,v in aaa.items():
                ...
                # elif 'neck' in k:
                #     newaaa[k] = v
            self.print_different_keys(newaaa, csd)
            ...
            for i, extra_teacher_path in enumerate(self.cfg.SSOD.extra_teachers):
                ...
                for na in self.cfg.SSOD.extra_teachers_class_names[i]:
                    ...
                    for idx, name in enumerate(self.cfg.Dataset.names):
                        if na == name:
                            curr_idx = idx
                    if len(self.cfg.SSOD.extra_teachers_class_names[
                               i]) == 1:  # teacher model是通过single-cls 训练的,这里 将原来的class-idx改为0
                        if self.RANK in [-1, 0]:
    ...
    def build_dataloader(self, cfg, callbacks):
        ...
        # Trainloader
        self.train_loader, self.dataset = create_dataloader(self.data_dict['train'], self.imgsz,
                                                            self.batch_size // self.WORLD_SIZE, gs, self.single_cls,
                                                            hyp=cfg.hyp, augment=True, cache=cfg.cache, rect=cfg.rect,
                                                            rank=self.LOCAL_RANK,
                                                            workers=cfg.Dataset.workers, prefix=colorstr('train: '),
                                                            cfg=cfg)
        ...
        self.unlabeled_dataloader, self.unlabeled_dataset = create_target_dataloader(self.data_dict['target'],
                                                                                     self.imgsz,
                                                                                     self.batch_size // self.WORLD_SIZE,
                                                                                     gs, self.single_cls,
                                                                                     hyp=cfg.hyp, augment=True,
                                                                                     cache=cfg.cache, rect=cfg.rect,
                                                                                     rank=self.LOCAL_RANK,
        ...
        if self.RANK in [-1, 0]:
            self.val_loader = \
                create_dataloader(self.data_dict['val'], self.imgsz, self.batch_size // self.WORLD_SIZE * 2, gs,
                                  self.single_cls,
    ...
    def update_train_logger(self):
        for (imgs, targets, paths,
             _) in self.train_loader:  # batch -------------------------------------------------------------
            imgs = imgs.to(self.device,
                           non_blocking=True).float() / self.norm_scale  # uint8 to float32, 0-255 to 0.0-1.0
    ...
    def train_in_epoch(self, callbacks):
        if (self.epoch < self.cfg.hyp.burn_epochs):
            ...
            if self.epoch == self.cfg.hyp.burn_epochs:
                msd = self.model.module.state_dict() if is_parallel(
                    self.model) else self.model.state_dict()  # model state_dict
                for k, v in self.ema.ema.state_dict().items():
                    ...
                    #     print('msd:', msd[k])
                if self.cosine_ema:
                    self.semi_ema = CosineEMA(self.ema.ema, decay_start=self.cfg.SSOD.ema_rate,
    ...
    def train_without_unlabeled(self, callbacks):
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if self.RANK in [-1, 0]:
                ...
                callbacks.run('on_train_batch_end', ni, self.model, imgs, targets, paths, self.plots, self.sync_bn,
                              self.cfg.Dataset.np)
        # end batch ------------------------------------------------------------------------------------------------
        # Scheduler
    ...
    def train_without_unlabeled_da(self, callbacks):
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + self.nb * self.epoch  # number integrated batches (since train start)
            imgs = imgs.to(self.device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
            target_imgs, target_targets, target_paths, _, target_imgs_ori, target_M = next(
                self.unlabeled_dataloader.__iter__())
            ...
            with amp.autocast(enabled=self.cuda):
                ...
                sup_pred, sup_feature, un_sup_pred, un_sup_feature = self.split_predict_and_feature(total_pred,
                                                                                                    total_feature,
                                                                                                    n_img)
                ...
                loss = loss + d_loss * self.da_loss_weights + t_loss * self.da_loss_weights + 0 * un_sup_pred[
                    0].mean() + 0 * un_sup_pred[1].mean() + 0 * un_sup_pred[2].mean()
                # else:
    ...
    def split_predict_and_feature_my(self, total_pred, total_feature, total_anchor, n_img):
        sup_feature = [total_feature[0][:n_img, :, :, :], total_feature[1][:n_img, :, :, :],
                       total_feature[2][:n_img, :, :, :]]
        un_sup_feature = [total_feature[0][n_img:, :, :, :], total_feature[1][n_img:, :, :, :],
                          total_feature[2][n_img:, :, :, :]]
        sup_anchor = [total_anchor[0][:n_img, :, :], total_anchor[1][:n_img, :, :],
                      total_anchor[2][:n_img, :, :]]
        un_sup_anchor = [total_anchor[0][n_img:, :, :], total_anchor[1][n_img:, :],
                         total_anchor[2][n_img:, :, :]]
        if self.model_type == 'yolov5':
            sup_pred = [total_pred[0][:n_img, :, :, :, :], total_pred[1][:n_img, :, :, :, :],
                        total_pred[2][:n_img, :, :, :, :]]
            un_sup_pred = [total_pred[0][n_img:, :, :, :, :], total_pred[1][n_img:, :, :, :, :],
                           total_pred[2][n_img:, :, :, :, :]]
            ...
            sup_pred = [[total_pred[0][0][:n_img, :, :, :], total_pred[0][1][:n_img, :, :, :],
                         total_pred[0][2][:n_img, :, :, :]], total_pred[1][:n_img, :, :], total_pred[2][:n_img, :, :]]
            un_sup_pred = [[total_pred[0][0][n_img:, :, :, :], total_pred[0][1][n_img:, :, :, :],
                            total_pred[0][2][n_img:, :, :, :]], total_pred[1][n_img:, :, :],
                           total_pred[2][n_img:, :, :]]
        ...
        return sup_pred, sup_feature, sup_anchor, un_sup_pred, un_sup_feature, un_sup_anchor
    ...
    def train_instance(self, imgs, targets, paths, unlabeled_imgs, unlabeled_imgs_ori, unlabeled_gt, unlabeled_M, ni,
        ...
        with amp.autocast(enabled=self.cuda):
            ...
            with torch.no_grad():
                if self.model_type in ['yolov5']:
                    (teacher_pred, train_out), teacher_feature = self.ema.ema(unlabeled_imgs_ori,
                                                                                       augment=False)
                # elif self.model_type == 'tal':
        ...
        if len(self.extra_teacher_models) > 0 and len(extra_teacher_outs) > 0:
            unlabeled_targets, unlabeled_imgs, invalid_target_shape = self.pseudo_label_creator.create_pseudo_label_online_with_extra_teachers(
                teacher_pred, extra_teacher_outs, copy.deepcopy(unlabeled_imgs), unlabeled_M,
                self.extra_teacher_class_idxs, self.RANK)
            ...
            unlabeled_targets, invalid_target_shape = self.pseudo_label_creator.create_pseudo_label_online_with_gt(
                teacher_pred, copy.deepcopy(unlabeled_imgs), unlabeled_M, copy.deepcopy(unlabeled_imgs_ori),
                unlabeled_gt, self.RANK)
        ...
        with amp.autocast(enabled=self.cuda):
            total_pred, total_feature = self.model(total_imgs)  # forward
            # sup_pred, sup_feature, sup_anchor, un_sup_pred, un_sup_feature, un_sup_anchor = self.split_predict_and_feature_my(
            #     total_pred,
            #     total_feature, total_anchor, n_img)
            #获取学生锚点


            #学生预测框展平
            # un_an = un_sup_anchor[0].tolist()
            # un_pre = un_sup_pred[0].tolist()
            sup_pred, sup_feature, un_sup_pred, un_sup_feature = self.split_predict_and_feature(
                total_pred,
                total_feature, n_img)
            ...
            else:  # 学生预测无标签结果与教师伪标签计算loss
                un_sup_loss, un_sup_loss_items = self.compute_un_sup_loss(un_sup_pred, unlabeled_targets.to(
                    self.device))  # 计算学生模型的训练和老师模型伪标签的loss
        ...
        if self.RANK in [-1, 0]:
            ...
            if invalid_target_shape:  # no pseudo label created
                ...
                if self.target_with_gt:
                    tp_rate, fp_cls_rate, fp_loc_rate, pse_num, gt_num = check_pseudo_label_with_gt(unlabeled_targets,
                                                                                                    unlabeled_gt, \
    ...
    def train_with_unlabeled(self, callbacks):
        ...
        if self.epoch_adaptor:
            ...
            for i, (target_imgs, target_gt, target_paths, _, target_imgs_ori, target_M) in pbar:
                ...
                target_imgs_ori = target_imgs_ori.to(self.device, non_blocking=True).float() / 255.0
                self.train_instance(imgs, targets, paths, target_imgs, target_imgs_ori, target_gt, target_M, ni, pbar,
                                    callbacks)
        else:
            pbar = enumerate(self.train_loader)
            if self.RANK in [-1, 0]:
            ...
            for i, (imgs, targets, paths, _) in pbar:
                ni = i + self.nb * self.epoch  # number integrated batches (since train start)
                target_imgs, target_gt, target_paths, _, target_imgs_ori, target_M = next(
                    self.unlabeled_dataloader.__iter__())
                imgs = imgs.to(self.device, non_blocking=True).float() / 255.0
                target_imgs = target_imgs.to(self.device, non_blocking=True).float() / 255.0
                target_imgs_ori = target_imgs_ori.to(self.device, non_blocking=True).float() / 255.0
    ...
    #
    # def get_anchors_one(self, featmap_sizes, device):

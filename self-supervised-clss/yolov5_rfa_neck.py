...
if __name__=="__main__":
    model = SimCLRStage1()
    state_dict = torch.load(r"D:\efficientteacher-main\self-supervised-pth\3900-350\best.pth")
    model.load_state_dict(state_dict,strict=False)
    # feature_extractor = create_feature_extractor(model, return_nodes=['f'])
    # nodes, _ = get_graph_node_names(model)
    # print(nodes)
    original_img = Image.open(r"G:\wkkSet\henanSet\allData\image_all\2018_2.png")
    model.eval()
    img = train_transform(original_img).unsqueeze(0)
    out = model(img)

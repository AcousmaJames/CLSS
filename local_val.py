...
def process_batch(detections, labels , path , iouv):
    """
    Return correct predictions matrix. Both sets of boxes are in (x1, y1, x2, y2) format.
    Arguments:
        detections (Array[N, 6]), x1, y1, x2, y2, conf, class
    ...
    imgName = os.path.basename(path[0]).split(".")[0]
    savePathR = os.path.dirname(path[0])
    out = 'out'
    savePathR = os.path.join(savePathR, out)
    if not os.path.exists(savePathR):
        os.makedirs(savePathR)
    txtFile = os.path.join(savePathR, imgName + ".txt")
    iou = box_iou(labels[:, 1:], detections[:, :4]) #相对于每个lable做IOU对比，返回lable个数*300个数据
    correct_class = labels[:, 0:1] == detections[:, 5]
    testTensor = iou.cpu().numpy()
    # matches = np.array([])
    x = torch.where((iou >= 0.5) & correct_class)  # IoU > threshold and classes match 返回对比label的数据，x[0]索引包含每个label，x[1]为对应的dete行标
    if x[0].shape[0]:
        ...
        if x[0].shape[0] > 1:
            ...
            with open(txtFile, "w") as f:
                for i in range(len(matches)):
                    xyxy = detections[int(matches[i][1])][:4]
                    context_cpu = xyxy.cpu()
                    context = str(context_cpu).replace("tensor", '').strip("()")
                    if i + 1 == len(matches):
                        f.write(context)
                    else:
                        f.write(context + '\n')
        ...
        with open(txtFile, "w") as f:
            f.write('')
    return torch.tensor(correct, dtype=torch.bool) #输出空的数据表，不用这个数组
...
def run(data,
        ...
        imgsz=2048,  # inference size (pixels)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if num_points > 0:
            ...
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls)# 300个预测框,list,bs里几个图片，len就是几
        dt[2] += time_sync() - t3
        # print(time_sync() - t3)

        # Statistics per image
        # a = 0
        for si, pred in enumerate(out):# 一次输入一张图片的300个预测框
            ...
            if nl:
                if num_points > 0 and val_kp:
                    ...
                    correct = process_batch(predn, labelsn, paths, iouv)
                    if plots:
        ...
        # print(a)
        # Plot images
        if plots and batch_i !=eval_num :
            f = save_dir / f'val_batch{batch_i}_labels.jpg'  # labels
            plot_images(img, targets, paths, f, 0, names)
            if val_kp:
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/wkk.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=r"D:\efficientteacher-main\runs\175-220-1024\rfa-self1900-350-et3\exp26\weights\best.pt", help='model.pt path(s)')
    ...
    parser.add_argument('--cfg', type=str, default=r'D:\efficientteacher-main\configs\sup\custom\yolov5l_custom.yaml', help='The config file used for validation')

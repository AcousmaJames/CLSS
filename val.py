...
def process_batch(detections, labels, iouv):
    ...
    correct_class = labels[:, 0:1] == detections[:, 5]
    for i in range(len(iouv)):
        x = torch.where((iou >= iouv[i]) & correct_class)  # IoU > threshold and classes match 返回对比label的数据，x[0]索引包含每个label，x[1]为对应的dete行标
        if x[0].shape[0]:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'th/best.pt', help='model.pt path(s)')

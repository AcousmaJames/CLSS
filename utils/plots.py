...
def plot_images(images, targets, paths=None, fname='images.jpg', num_points=0, names=None,  max_size=1920, max_subplots=16):
    ...
    # Build Image
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init画布，画布要容下每个bs的数据
    for i, im in enumerate(images):
        if i == max_subplots:  # if last batch has fewer images than we expect
            break
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
...
def feature_visualization_all(x, n,save_dir=Path('runs/detect/exp')):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
    ...
    if height > 1 and width > 1:
        f = save_dir / f"stage_back_1900-350_features{n}_8.png"  # filename

...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(1, 3), e=0.5,
                 act=True):  # ch_in, ch_out, shortcut, groups, expansion, act
        super().__init__()
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5,
                 act=True):  # ch_in, ch_out, number, shortcut, groups, expansion, act
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class ELAN(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5, with_mp=True, with_aug=True,
                 act=True):  # ch_in, ch_out, number, groups, expansion, act
        super().__init__()
...
class Bottleneck_RFAConv(Bottleneck1):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e) # 128 128
        c_ = int(c2 * e)  # hidden channels # 64
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = RFAConv(c_, c2, k[1])# 64 128


class C3_RFAConv(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5): #128 256 n1
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels #128 128 n1
        self.m = nn.Sequential(*(Bottleneck_RFAConv(c_, c_, shortcut, g, k=(1, 3), e=1.0) for _ in range(n)))


if __name__ == "__main__":
    x = torch.randn(1, 32, 64, 64)
    model = RFAConv(32, 32, 3)
    # # 生成模型的计算图
    # y = model(x)
    # dot = make_dot(y, params=dict(list(model.named_parameters())))
    # dot.render("C3_RFAConv_model", format="png")
    model_graph = draw_graph(model, input_size=x.shape, expand_nested=True, save_graph=True, filename="torchview",
                             directory=".")

...
from models.detector.litong_tezhengtu import draw_features, draw_features2
...
class Model(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        f8, f16, f32 = x
        # self.save_4d_tensor_to_csv(f8, r'D:\efficientteacher-main\models\detector\tensor_1')
        # self.save_4d_tensor_to_csv(f16, r'D:\efficientteacher-main\models\detector\tensor_2')
        # self.save_4d_tensor_to_csv(f32, r'D:\efficientteacher-main\models\detector\tensor_3')
        ...
        # feature = [f8, f16, f32]
        # 基于 YOLOv5 输出的置信度生成热图
        outname = r"G:\wkkSet\henanSet\retunew\retu_rfa-1185-875-010-29-8\back"
        draw_features(x1[0],outname)
        # outname2 = r"G:\wkkSet\henanSet\retunew\retu_rfa-1900-350-010-58-8-2\back"
        # draw_features2(x1[0], outname2)

        # if visualize:
        #     feature_visualization_all(x[0],1)
        input_image = cv2.imread(r"G:\wkkSet\henanSet\allData\image_all\2016_29.png")   # 传入原始图像
        heatmap_image = self.generate_heatmap(input_image, out)
        cv2.imwrite(r"D:\efficientteacher-main\models\detector\heatmap_output4.jpg", heatmap_image)
        return out, feature
    ...
    def generate_heatmap(self, image, out, conf_threshold=0.005):
        """
        基于输出的置信度生成热图
        :param image: 输入的原始图像 (H, W, C)
        :param out: YOLOv5 格式的输出，形状为 [batch_size, num_detections, 6]
        :param conf_threshold: 置信度阈值
        """
        heatmap = np.zeros((image.shape[0], image.shape[1]))  # 初始化热图，大小与图像相同

        # 遍历所有检测
        for detection in out[0][0]:  # 假设 batch size 为 1
            x, y, w, h, conf, cls = detection #yolo格式的是
            x = x.item()
            y = y.item()
            w = w.item()
            h = h.item()
            conf_scalar = conf.item()
            cls_scalar = cls.item()
            # 过滤掉低置信度的检测框
            if conf_scalar < conf_threshold:
                continue
            ...
            x_max = x + w / 2
            y_max = y + h / 2
            heatmap[int(y_min):int(y_max), int(x_min):int(x_max)] += conf_scalar
        ...
        # plt.colorbar(label='Confidence Score')  # 添加颜色条，表示置信度的强弱
        # plt.title('Heatmap of Detection Confidence')
        # plt.show()
        heatmap2 = np.clip(heatmap2, 0, 1)
        heatmap2 = np.uint8(255 * heatmap2)
        # 将热图叠加到原始图像上
        heatmap_img = cv2.applyColorMap(heatmap2, cv2.COLORMAP_JET)
        superimposed_img = cv2.addWeighted(image, 0.5, heatmap_img, 0.5, 0)

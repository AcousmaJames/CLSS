...
from ..backbone.common import Conv, Concat, C3, C3_RFAConv
from utils.general import make_divisible


class YoloV5RfaNeck(nn.Module):
    """
    ...
    def __init__(self,cfg):
        super(YoloV5RfaNeck, self).__init__()
        ...
        input_p3, input_p4, input_p5 = in_channels
        output_p3, output_p4, output_p5 = out_channels
        ...
        self.C1 = C3_RFAConv(int(self.input_p5 / 2) + self.input_p4, self.input_p4, self.get_depth(3), False, 1, 0.5)

        self.conv2 = Conv(self.input_p4, self.input_p3, 1, 1, None, 1, CONV_ACT)
        self.upsample2 = nn.Upsample(scale_factor=2, mode="nearest")

        self.C2 = C3_RFAConv(self.input_p3 + self.input_p3, self.output_p3, self.get_depth(3), False, 1, 0.5)
        self.conv3 = Conv(self.output_p3, self.output_p3, 3, 2, None, 1, CONV_ACT)

        self.C3 = C3_RFAConv(self.output_p3 + self.input_p3, self.output_p4, self.get_depth(3), False, 1, 0.5)
        self.conv4 = Conv(self.output_p4, self.output_p4, 3, 2, None, 1, CONV_ACT)

        self.C4 = C3_RFAConv(self.output_p4 + int(self.input_p5 / 2), self.output_p5, self.get_depth(3), False, 1, 0.5)
